...
def calc_spatial_cor(vel_time, cell_size, time):
    ...
    avg_vel = np.mean(vel_time[:,3:6], axis = 0)
    ...
    for ii in range(len(tags)):
        ...
        vel_x = vel_time[ii:,3] - avg_vel[0]
        vel_y = vel_time[ii:,4] - avg_vel[1]
        vel_z = vel_time[ii:,5] - avg_vel[2]

        vel_dot=vel_x*(vel_time[ii,3]-avg_vel[0])+vel_y*(vel_time[ii,4]-avg_vel[1])+vel_z*(vel_time[ii,5]-avg_vel[2])
